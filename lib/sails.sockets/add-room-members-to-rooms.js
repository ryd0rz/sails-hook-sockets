...
module.exports = function(app) {
  ...
  return function addRoomMembersToRooms (sourceRoom, destRooms, cb) {
    ...
    if (_.find(Array.from(app.io.sockets.sockets), ([mapKey]) => mapKey == sourceRoom)) {
      return doJoin(_.find(Array.from(app.io.sockets.sockets), ([mapKey]) => mapKey == sourceRoom), cb);
    }
    ...
    // Look up all members of sourceRoom
      ...
      let sourceRoomSocketIds = _.map(Array.from(app.io.sockets.sockets), _.pick('socketId'));
      return async.each(sourceRoomSocketIds, function(socketId, nextSocketId) {
        ...
        if (_.find(Array.from(app.io.sockets.sockets), ([mapKey]) => mapKey == socketId)) {
          ...
          return doJoin(_.find(Array.from(app.io.sockets.sockets), ([mapKey]) => mapKey == socketId), nextSocketId);
        }
