...
module.exports = function(app) {
  ...
  return function joinRoom (sockets, roomName, cb, disconnectCB) {
    ...
    async.each(sockets, function(socket, nextSocket) {
      ...
      if (typeof socket !== 'object' && socket !== null) {
        ...
        if (!_.find(Array.from(app.io.sockets.sockets), ([mapKey]) => mapKey == socket)) {
          return app.sockets.addRoomMembersToRooms(socket, roomName, nextSocket);
        }
        // Otherwise get the socket object and continue
        socket = _.find(Array.from(app.io.sockets.sockets), ([mapKey]) => mapKey == socket)[1];
      }
      ...
      if (disconnectCB && typeof disconnectCB === 'function') {
        socket.on("disconnect", function(reason) {
          console.log('DISCONNECT DETECTED on socket id ' + socket.id + '. Reason:' + reason)
          disconnectCB();
        });
      }
